    ...
    func fetchData(url: URLRequest) {
            session.dataTask(with: url) { [weak self] data, response, error in
                ...
                do {
                    ...
                    self?.delegate?.transferData(self!, data: characters)
                } catch {
    ...
    func characterSearchRequest(_ url: URL) {
        session.dataTask(with: url) { [weak self] data, response, error in
            ...
            do {
                ...
                let character = baseData.results
                self?.delegate?.transferData(self!, data: character)
            } catch {
                print(error.localizedDescription)
            }
        }.resume()
    ...
    func episodeSearchRequest(_ url: URL) {
        session.dataTask(with: url) { [weak self] data, response, error in
            ...
            do {
                ...
                let episode = baseData.results.map({ $0 })
                self?.delegate?.searchEpisodeData(self!, data: episode)
            } catch {
                print(error.localizedDescription)
            }
        }.resume()
    ...
    func locationSearchRequest(_ url: URL) {
        session.dataTask(with: url) { [weak self] data, response, error in
            ...
            do {
                ...
                let location = baseData.results.map({ $0 })
                self?.delegate?.searchLocationData(self!, data: location)
            } catch {
                print(error.localizedDescription)
            }
        }.resume()
